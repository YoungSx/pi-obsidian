...
| **你的端点，你的密钥** | 任何 OpenAI 兼容或 Anthropic-messages 的 base URL，内置九家服务商建议，能力字段自动填 —— [怎么配](docs/settings.zh-CN.md#模型) |
| **图片** | 粘贴或拖进输入框，随消息一起走 |
...
Piem 的 `isDesktopOnly` 是 `false`，而且是当真的。流式输出、工具、子代理、
技能、图片——在手机上全都能用。

这份承诺有代价，而且值得让你知道代价是什么：MCP 服务器**只支持远程**，因为
stdio 传输要拉起子进程，手机做不到。一个在手机上不可能存在的能力，宁可对所
有人都不做，也不做成桌面端的意外惊喜。
