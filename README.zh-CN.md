...
Piem 是 agent 插件：要把活干成，就必须动用社区插件官方审查会显式报告的几类
能力，所以这里按同一套口径逐条列出。它们都不是缺陷或绕路——每一项都支撑着
某个功能；除上文所述之外，不存在任何回传。

- **直接文件系统访问。** 桌面端插件经 Electron 的 `require` 触达 Node `fs`，
  绕过 vault API。这正是用户级技能目录（`~/.pi/agent/skills`、
  `~/.agents/skills`）读写的实现方式：vault API 看不到仓库之外。没有 shell
  执行，也不探测任意路径——实际访问的是用户主目录及其下的技能目录。移动端
  宿主不暴露 `require`，此路径会降级为「不可用」而不是拖垮插件。
- **全库枚举。** 搜索与任务工具调用 `vault.getFiles` /
  `vault.getMarkdownFiles`，会列出全库每个文件及其完整路径。模型在搜索或任务
  查询触及时就能看到这些列表——这是「搜我的仓库」的固有属性，也正因如此，
  指给 Piem 的仓库应当是你愿意发给模型服务商的仓库。
- **剪贴板访问。** 只有写出方向：回复、日志行、设置里的「复制密钥」会调用
  `navigator.clipboard.writeText`。Piem 从不读剪贴板；粘贴的图片走的是编辑器
  自身的 paste 事件，不是剪贴板读取。
- **运行时 base64 编码。** 图片附件在作为模型内容发送前用 `btoa` 编码。仅此
  一处用途——没有任何对混淆载荷的解码。
- **网络请求。** 约二十处调用点，全部经由同一传输层：模型服务商流式请求、
  连接测试、models.dev 目录建议、远程 MCP 服务器，以及默认关闭的 `webfetch`
  工具。传输层遵循用户选定的 `requestUrl`-vs-`fetch`，出口集中在一处可审查，
  而不是散落各处。
