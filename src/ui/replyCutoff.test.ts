...
describe("describeReplyCutoff", () => {
	...
	it("reports a reply the reader's own next message cut short, not as a stop", () => {
		// Same `stopReason` as the stop button — pi has no other way to record an
		// abort — so without the stamp the transcript would tell someone who typed
		// a correction that they pressed stop (issue #289).
	...
	it("translates every notice", () => {
		const steered = reply("aborted");
		markReplySteered(steered);
		expect(describeReplyCutoff(steered, zh)?.notice).toBe("你的下一条消息打断了这条回复。");
