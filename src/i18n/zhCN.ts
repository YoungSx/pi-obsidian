...
export const zhCN: DeepPartial<EnCopy> = {
	...
	chat: {
		...
		laneOption: "方案 {label}",
		laneChoose: "就用这条",
		laneChooseTitle: "定稿这条分支？",
		laneChooseBody: "这条分支将成为正式对话。另一条怎么处理？",
		laneChooseKeep: "留作参考",
		laneChooseRetire: "清理掉",
		editingCancel: "取消编辑",
		replyTruncated: "这条回复达到模型的长度上限，提前结束了。",
		replyTruncatedSpoken: "这条回复达到模型的长度上限，提前结束了。",
		providerFailure: {
			auth: "供应商拒绝了这个密钥。到设置里核对一下再问。",
			authSpoken: "供应商拒绝了这个密钥。",
			quota: "这个账号在供应商那边额度用完了。充值后再问。",
			quotaSpoken: "这个账号在供应商那边额度用完了。",
			contextLength: "这段对话超出了模型能装下的长度。整理较早的消息后再问。",
			contextLengthSpoken: "这段对话超出了模型能装下的长度。",
			refused: "供应商拒绝回答这一条。换个说法通常就好了。",
			refusedSpoken: "供应商拒绝回答这一条。",
			rateLimit: "供应商正在限流，或者忙不过来。等一下再试。",
			rateLimitSpoken: "供应商正在限流，或者忙不过来。",
			timeout: "供应商没在规定时间内回话。",
			timeoutSpoken: "供应商没在规定时间内回话。",
			offline: "连不上供应商。检查一下网络再试。",
			offlineSpoken: "连不上供应商。",
			serverError: "供应商自己出错了。再试一次通常就好。",
			serverErrorSpoken: "供应商自己出错了。",
...
5. 只有用户明确要求安装时，才获取并审查完整 SKILL.md，然后写入 Piem/skills/<name>/SKILL.md。不要执行远程代码，不要复制隐藏脚本，覆盖已有 vault skill 前必须确认。`,
	...
	mcp: {
		...
		// 只在选了缓冲传输时出现——它描述的就是那一种状态。这条降级是真的，
		// 但此前只写在源码注释里，用户看不见的地方。文案点名传输和它所在的
		// 标签页，是为了让读者能去关掉它，而不只是知道有这回事。
		bufferedNoPush:
			"在缓冲传输下，服务器无法主动推给你：工具列表只在你保存设置时刷新，服务器自己改了不算。想拿回服务端推送，去「模型」里把传输换成 fetch。",
		add: "添加服务器",
		edit: "编辑",
		delete: "删除",
	...
	subagents: {
		...
		processNone: "什么都没留下：这次运行在过程记录被保存之前就失败了。",
		line: {
			user: "指令",
			assistant: "回复",
			thinking: "思考",
